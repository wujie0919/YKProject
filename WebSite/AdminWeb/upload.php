...
//echo "Upload: " . $_FILES[$upload_name]["tmp_name"] . "<br />";
	...
	$extension_whitelist = array("MPEG", "AVI","avi","MOV","ASF","WMV","3GP","FLV","flv","RMVB","mp4","MP4","PNG","png");	// Allowed file extensions
...
//echo $file_name;
    $tool=new Tools();
    $file_name = $tool->getCommonId().".".$tool->getFileType($_FILES[$upload_name]['name']);
...
// Validate that we won't over-write an existing file
	if (file_exists($save_path . $file_name)) {
        //
       if( insertDB($file_name))
            echo"<script>history.go(-1);alert('上传成功'); window.location.href=\"http://localhost:8888/WebSite/AdminWeb/vedioAddAndUpdate.php?type=add\";
</script>";
...
//		exit(0);
//        history.back();
        echo"<script>history.go(-1);alert('上传失败');</script>";
...
function insertDB($url)
{
    ...
    $selectCity = $_POST["select"];
    $comment = $_POST["comment"];
    $urls = "/WebSite/AdminWeb/uploads/" . $url;
    $uploadDate = date('Y-m-d H:i:s');

// CREATE TABLE  if not exists YK_Video(videoId VARCHAR (20) PRIMARY KEY ,
//videoAddress VARCHAR (10),videoName VARCHAR (50),videowner VARCHAR (20),uploadDate VARCHAR (20),videoDesc VARCHAR (200),videoStatus VARCHAR (2))
    ...
    $pass = md5($_POST["pass"]);
    $sql = "INSERT INTO YK_Video(videoAddress,videoName,videowner,uploadDate,videoDesc,videoStatus,videoArea)VALUES('$urls','$name','10000','$uploadDate','$comment','0','$selectCity')";
